...
[@ferrariFusingSentinel1Sentinel22023] use Fully Convolutional Networks [@longFullyConvolutionalNetworks2015] for fusion of optical Sentinel-2 and SAR Sentinel-1 data.
...
[@nilssonEstimationTreeHeights1996; @naessetDeterminationMeanTree1997; @naessetEstimatingTimberVolume1997; @carson2004lidar].
...
[@weinmannFeatureRelevanceAssessment2013] show that careful selection of features is crucial for accurate and efficient interpretation of point cloud data.
...
[@zhangImprovedAreabasedApproach2023] use a modification of the area-based approach to predict plot-level diameter at breast height (dbh) by utilizing known allometric dependencies between tree height as a hypothesis set for fitting statistical models.
...
[@vermeerLidarbasedNorwegianTree2023] uses a U-Net [@ronnebergerUNetConvolutionalNetworks2015] image semantic segmentation model on LiDAR-derived 1-meter resolution digital terrain models and canopy height maps to predict the distribution of three main tree species in a Norwegian forest.
...
[@lucasIdentificationLinearVegetation2019] uses the set of eigenvalue-based features calculated for each point in a fixed-radius neighborhood, with other geometrical features including maximum local height difference and height standard deviation, local radius and local point density.
They also use two point-based features that don't rely on a neighborhood: the number of returns and the normalized return number, proposed in [@guoRelevanceAirborneLidar2011].
...
[@venturaIndividualTreeDetection2024] built a network they call HR-SFAnet, consisting of a VGG-16 [@simonyanVeryDeepConvolutional2014] backbone feature extractor, a confidence head, and a parallel attention head, which is a deeper modification of the SFANet proposed in [@zhuDualPathMultiScale2019] for counting the amount of people in photos, for detecting individual trees in urban environments using high-resolution multispectral images.
...
[@doussExtractionIndividualTrees2022] offers an exploration of how different window size functions for the local maxima filtering applied to LiDAR-derived canopy height maps affect the quality of individual tree detection.
