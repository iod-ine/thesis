...
The architecture chosen to serve as the tree segmentation network is the PointNet++ [@qiPointNetPlusPlus2017], described in detail in @sec-deep-learning.
...
However, the main goal of this thesis was to develop and verify an overall framework, and thus the choice was set on a model that is simple to implement and work with to allow easily experimenting with other parts of the proposed system.

The architecture of the used PointNet++ is similar to the segmentation architecture shown in @fig-pointnet2-architecture.
The main differences are the usage of one more stacked set abstraction layer to make the network deeper, and instead of using a classification head that predicts per-point class scores it uses a regression head that predicts a continuous value for each point, since it needs to assign a unique ID to every tree.
Architecture uses three consecutive set abstraction blocks with proportions of selected points set to 0.75, 0.5, and 0.5 and neighborhood radii set to 0.1, 0.2, and 0.4.
Note that the scale of the input is normalized, so the radii are not in meters.
Total of 30 million trainable parameters.

The batch size is limited by the available GPU device memory, and in the described setup directly competes with the synthetic forest patch dimensions used in the training dataset.
The preference is given to the size of the patch, and the batch size is set to 1, but is compensated for by using gradient accumulation steps – updates to the model parameters are made after the gradient is accumulated for a set number of iterations.
...
The augmentations are described in detailed with visualized examples in @sec-augmentations.
...
### Augmentations {#sec-augmentations}
