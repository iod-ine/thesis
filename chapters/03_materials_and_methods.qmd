...
The continuous prediction of the regression model are rounded to get the integer labels for every point.
...
It traces the overlapping points to find the labels that need to be updated in the later window, and adds a number larger than there could be trees within a patch of that size to predictions with remaining labels of the later window.
Second, the segments identified by predicted integer labels are extracted, selected feature sets are calculated for each one, each one is passed through the collection of attribute prediction models to get per-tree predictions.
