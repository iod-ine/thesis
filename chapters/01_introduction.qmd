...
@fig-forest-coverage offers a map of the global tree coverage based on data from @hansenHighResolutionGlobalMaps2013 to highlight the extent of forests on the planet.
...
The traditional manual forest inventories that rely on people going out into the forest to count and measure trees will always remain the most reliable and accurate way to assess forest parameters [@burleyEncyclopediaForestSciences2004].
...
::: {#fig-forest-coverage}
![](../images/global-forest-cover.jpg)

Global tree cover for 2010.
Area occupied by tree canopies relative to the total area of the pixel.
Data from @hansenHighResolutionGlobalMaps2013
:::

Many national space agencies provide open access to satellite data that can be used for mapping forests.
European Space Agency offers free and open access[^1] to the Sentinel missions through its [Copernicus Data Space Ecosystem](https://dataspace.copernicus.eu/) platform, including C-band SAR data from Sentinel-1 and medium-resolution multispectral data from Sentinel-2, both with global coverage.
They also have an upcoming P-band SAR mission called BIOMASS, designed specifically to study global forest biomass and carbon cycles [@queganEuropeanSpaceAgency2019], which further indicates the growing importance and interest in the topic of forest mapping.
NASA provides free access to an abundance of satellite data through its [Earthdata platform](https://www.earthdata.nasa.gov/), Landsat mission with the Operation Land Imager (OLI) instrument and Terra mission with the Moderate Resolution Imaging Spectroradiometer (MODIS) instrument being the most relevant for forest mapping applications.
The Indian Space Research Organisation (ISRO), Brazilian National Institute for Space Research, National Space Research and Development Agency of Nigeria all offer free satellite data that can be used for this purpose.
NASA and the ISRO also have a joint upcoming mission called NISAR with two fully-polarimetric SAR sensors at L-band and S-band [@kelloggNASAISROSyntheticAperture2020], which will benefit forestry applications greatly.

[^1]: Although the access is not open for everyone equally, as I have observed silent bans of the accounts connecting from Russian IP addresses without any response to support inquires in the previous platform, and an outright IP ban in the current one.
...
It is easy to use, but its results are often not detailed enough when working on finer scales.
...
The task is relatively[^2] easy in urban environments, manually planted and managed forest stands, or forests that are either sparse or predominantly coniferous, where the structure of the canopy is easy to interpret.
...
[^2]: The word relatively does a lot of heavy lifting here, as the problem is by no means easy on its own and takes a lot of effort from many researchers to continue to make progress on.

The framework described in this thesis focuses on the fusion of two remote sensing data sources, UAV LiDAR point clouds and UAV RGB orthophotos, to detect individual trees in dense mixed forests and predict required attributes for each tree individually, producing the most detailed maps possible.
...
According to the Design Research Methodology framework [@blessingDRMDesignResearch2009], this research project can be classified as type 3: Development of Support.
...
## Research question and hypothesis {#sec-research-question}
...
The proposed framework consists of a neural network-based tree segmentation in UAV LiDAR point clouds enhanced with RGB orthophoto-based features, with further processing of the segments using a collection of specialized classic machine learning models that predict the parameters of interest for each detected tree.
The tree segmentation model is trained on synthetic forest patches constructed from a dataset of point clouds of individual trees extracted manually from a large UAV LiDAR survey, heavily relying on augmentations to make the synthetic forest closer to the real forest.
...
In fact, as mentioned in @sec-improvements, such replacement are encouraged to improve the system.
...
The inputs for the framework are a UAV LiDAR point cloud and an RGB orthophoto.
...
Each individual node is described in detail in @sec-materials-and-methods.
To create a working implementation, a manual forest inventory covered by a UAV LiDAR point cloud and an RGB orthophoto are required.
...
The resulting trained models are then used for inference as shown in @fig-framework-apply.
:::
...
@sec-introduction aims to give a general introduction to the research project and put it into a wide scientific and societal context.
It defines the main research question and the hypothesis, and gives a high-level overview of the proposed framework.
It also provides the links to the original datasets and the code.
...
Its aim it to make the work reproducible and to explain the methodological choices made.
...
@sec-conclusion offers a brief summary of the thesis as a whole, potential approaches for further improvement of the proposed framework, and some concluding thoughts.
...
An HTML version of this thesis is hosted through GitHub Pages and is available [here](https://iod-ine.github.io/thesis).
The thesis document was developed using Quarto [@Allaire_Quarto_2024] using the literate programming approach [@knuth84].
