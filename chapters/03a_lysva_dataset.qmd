...
As an example, @fig-intensity-with-example shows the distribution of intensity values for every point in the Lysva dataset, and shows plot number 10 in 3D colored by intensity.
The distribution of intensities seems like an artifact of faulty quantization (the fact that the maximum overall value is 63 makes me suspect it is stored by the hardware using a 6-bit unsigned integer, probably an ad hoc optimization by the vendor).
With this distribution in mind, it is not surprising that visualizing the intensities in 3D scatter plot of the ground plots results in images that look like noise.

Some sensors provide more consistent values.
However, it limits the relying on intensity-based features limits the applicability of developed models and methods, as any such model would surely fail on data like this.
