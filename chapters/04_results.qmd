...
The mean X and Y coordinates of the segmented trees were used to convert the cloud into a set of points, where each point represents a candidate detected tree.
Then the algorithm described in @sec-matching-algorithm is applied to match the detected trees to the ground truth trees.
Then precision, recall, and $F_1$-score were calculated for the detection results, and for the true positive matches the species classification accuracy and diameter at breast height regression error are calculated.

@tbl-results shows the tree detection metrics
