...
As was mentioned in @sec-other-training-params, during training model checkpointing is set up to track average validation accuracy – the proportion of points for which the final predicted label is correct.
...
### Effect of dropout augmentation
...
The most important augmentation that addresses the selection bias present in the dataset of individual tree point clouds is the height-dependent dropout described in @sec-augmentations.
...
Examples of threshold curves for different values of the parameters are shown in @fig-height-dropout and @fig-height-dropout-aggressive.
Milder variations, which are characterized by lower positions of the incline and more gradual slopes, as in @fig-height-dropout, have similar effects to not using the augmentations at all.
...
As the dropout becomes more aggressive, with the position of the incline higher and slopes steeper, as in @fig-height-dropout-aggressive, the training data becomes more similar to the real data.
...
@fig-dbh-distribution show the distributions of the field measured dbh in the full Lysva dataset for reference.
On the top it show a boxplot with every individual observation plotted as a point, jittered along the Y axis to reduce overlap.
...
{{< embed 03a_lysva_dataset.ipynb#fig-dbh-distribution >}}
...
This approach thus outperforms the basic individual local maximum filter in tree detection in the Lysva region as reported in @dubrovinOpenDatasetIndividual2024 by 0.13.
